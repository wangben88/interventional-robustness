...
std::pair<Cnf, Lmap> loadCnfSpecial(std::string infile, Cnf classifierCnf, std::string orderingfile, std::string constraintfile) {
    ...
    bool orderingSpecified = !orderingfile.empty();
    ...
    */
    ...
    // Read ordering
    int numSrcVarsOrderingFile;

    std::cout << orderingfile << std::endl;
    if (orderingSpecified) {
        std::ifstream fin2(orderingfile);

        fin2 >> numSrcVarsOrderingFile;
        ordering.resize(numSrcVarsOrderingFile);
        std::cout << ordering.size() << std::endl;
        for (int srcVarIdx = 0; srcVarIdx < numSrcVarsOrderingFile; srcVarIdx++) {
            fin2 >> ordering[srcVarIdx];
        }
        fin2.close();
    ...
    while (firstPart == "c") {
        ...
        if (secondPart == "Variables") {
            ...
            iss >> numSrcVars;
            if ((orderingSpecified) && (numSrcVars != numSrcVarsOrderingFile)) {
                throw std::logic_error("Error: Number of variables doesn't match in ordering file and CNF file");
            }
        }
    ...
    std::cout << numClauses << " " << numVars << std::endl;
    ...
    for (int srcVarIdx = 0; srcVarIdx < numSrcVars; srcVarIdx++) {
        ...
        // if ordering not specified, fill ordering with the default ordering
        if (!orderingSpecified) {
        ...
        std::cout << srcVarName << std::endl;
        std::cout << indices[0] << std::endl;
        std::cout << indices[1] << std::endl;
        std::cout << srcVarIdx << std::endl;
    }

    fin.close();
    std::cout << "HODIY " << maxIndicatorVarIdx << std::endl;

    // If ordering set explicitly, work out the mapping from original indices in CNF file to desired indices
    // Then adjust the CNF clauses.
    std::vector<long long> origToDesiredIdxMap(numVars); // original to desired
    for (int i = 0; i < numVars; i++) {
        origToDesiredIdxMap[i] = i; // by default, keep the index the same. In particular, parameter AC vars will retain the same index;
    }
    if (orderingSpecified) {
        int newIdx = 0;
        for (const std::string& srcVarName: ordering) {
            for (long long &cnfIdx: srcVarNameValToIndicatorNodeIndex[srcVarName]) {
                origToDesiredIdxMap[cnfIdx] = newIdx;
                cnfIdx = newIdx; // update index in srcVarNameValToIndicatorNodeIndex
                newIdx++;
            }
        }
        for (auto& clause: clauses) {
            clause.remapLiterals(origToDesiredIdxMap);
        }
    }
    ...
    bool constraintSpecified = !constraintfile.empty();
    if (constraintSpecified) {
        ...
        for(auto pr: combinedCnfConstraintMap) {
            std::cout << pr.first << std::endl;
            for (auto xh: pr.second) {
                std::cout << xh << " ";
            }
            std::cout << std::endl;
        }
        ...
        for(auto pr: cnfOptimalOrdering) {
        ...
        std::cout << "NEW ORDERING 3" << std::endl;
        for(auto pr: oldOrderingToNewOrdering) {
            std::cout << pr << std::endl;
        }
